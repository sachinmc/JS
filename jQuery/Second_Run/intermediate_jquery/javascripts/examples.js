// delegated Events
// namespacing events
// events bound with options object
// create events
...
$(function() {
  ...
  function displayParentChildRelationship(e) {
    //console.log(this, e.target, e.currentTarget, e.delegateTarget);
  ...
  // can also pass in data to the event callbacks
  // useful for functions that might handle mutiple events, so you can
  // specify data unique to that event callback.
  $("#namespaced").on("click.alert", { event_namespace: "alert" }, function(e) {
    // access the data object on the event
    console.log(e.data.event_namespace); // gets console log of 'alert'
    $(this).off("click.alert");
  });
  ...
  //$("#parent").on("click", displayParentChildRelationship.bind(window));
