function sentiment(text) {
  ...
  negativeWords.forEach(function (word) {
    ...
    var matched = text.match(regex);
    if (matched !== null) {
      negative.push(matched);
    }
  });

  positive = positive.reduce(function (acc, curr) {
    return acc.concat(curr);
  })

  negative = negative.reduce(function (acc, curr) {
    return acc.concat(curr);
  })

  formattedOutput(positive, negative);
}

function formattedOutput(positives, negatives) {
  var textSentiment;
...
function sentiment(text) {
  ...
  formattedOutput(positives, negatives);
}
